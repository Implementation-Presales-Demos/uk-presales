  ...
  if (result.action) {
      ...
      switch (result.resultCode) {
          ...
          case 'Authorised':
              ...
              window.demoSession.enableEcom_adyenGiving === "off" ? globalCheckout.create('donation', donationConfig).mount('#donation-container') : null;
              break;
          default:
...
const translations = {
    "en-GB": {
        "confirmPreauthorization": "Save card",
        "payButton": "Pay",
        "creditCard.success": "Card Successful Saved"
    }
};
    ...
    checkoutApi.getPaymentMethods(paymentDataObj, window.demoSession.allowedPaymentMethods).then(async function (paymentMethodsResponse) {
        ...
        let checkoutConfig = {
            ...
            onSubmit: function (state, dropin) {
                ...
                checkoutApi.makeStoreCardPayment(state, dropin).then(function (result) {
        ...
        let applepay = globalCheckout.create("applepay", {
          ...
          onAuthorized: (resolve, reject, event) => {
              ...
              checkoutApi.submitStoreCardPayment(localState).then(function (result) {
